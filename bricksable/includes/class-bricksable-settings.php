...
class Bricksable_Settings {
	...
	/**
	 * Get Bricks Builder Elements.
	 *
	 * @var     array
	 * @access  public
	 * @since   1.6.54
	 */
	public $bricksbuilder_elements = array();
	...
	public function __construct( $parent ) {
		...
		// Manage Element.
		add_filter( 'bricks/builder/elements', array( $this, 'bricksbuilder_elements' ) );
	...
	private function settings_fields() {
		...
		$fields = array(
			array(
				'id'          => 'all_elements',
				'description' => __( 'Check to select all Bricksable Elements.', 'bricksable' ),
				'type'        => 'checkbox',
				'default'     => 'on',
			),
		);
		...
		$settings['elements'] = array(
			'title'  => __( 'Bricksable Elements', 'bricksable' ),
			'fields' => $fields,
		);

		$settings['bricksbuilder_elements'] = array(
			'title'  => __( 'Bricks Builder Elements', 'bricksable' ),
			'fields' => array(
				array(
					'id'          => 'all_bricks_builder_elements',
					'description' => __( 'Check to select all Bricks Builder Elements.', 'bricksable' ),
					'type'        => 'checkbox',
					'default'     => 'off',
				),
				array(
					'id'          => 'bricks_builder_elements',
					'label'       => __( 'Bricks Elements', 'bricksable' ),
					'description' => __( 'You can deactivate or activate Bricks elements. Streamlining your builder interface and making it quicker to find the elements you need.', 'bricksable' ),
					'type'        => 'checkbox_multi',
					'options'     => $this->get_bricksbuilder_elements(),
					'default'     => $this->get_bricksbuilder_elements_array(),
				),
			),
		);
	 ...
	 *
	 * @param return $bricksbuilder_elements Get the Elements.
	 */
	public function bricksbuilder_elements( $bricksbuilder_elements ) {
		$this->bricksbuilder_elements = $bricksbuilder_elements;
		if ( bricks_is_builder() ) {
			$bricks_builder_element_options = get_option( 'bricksable_bricks_builder_elements' );
			if ( false === $bricks_builder_element_options ) {
				return $this->bricksbuilder_elements;
			}
			if ( ! empty( $bricks_builder_element_options ) ) {
				$elements_include = array(
					'container',
					'section',
					'block',
					'div',
				);
				$elements_merge   = array_merge( $elements_include, $bricks_builder_element_options );

				return array_intersect( $bricksbuilder_elements, $elements_merge );
			}
		}
		return $bricksbuilder_elements;
	}
	/**
	 * Get Bricks Builder Elements
	 */
	public function get_bricksbuilder_elements() {

		$bricksbuilder_elements = array();
		$label                  = array();
		$uppercase_label        = array();
		$elements_exclude       = array(
			'container',
			'section',
			'block',
			'div',
		...
		foreach ( $this->bricksbuilder_elements as $key => $element ) {
			$bricksbuilder_elements[] = $element;
			if ( 'post-toc' === $element ) {
				$label[] = __( 'Table of contents', 'bricksable' );
			} else {
				$label[] = $element;
			}
		}

		$exclude_elements = array_diff( $bricksbuilder_elements, $elements_exclude );
		$exclude_labels   = array_diff( $label, $elements_exclude );
		foreach ( $exclude_labels as $key => $label ) {
			$uppercase_label[] = ucwords( str_replace( '-', ' ', $label ) );
		}

		$bricksbuilder_elements = array_combine( $exclude_elements, $uppercase_label );
	 ...
	 * Get Bricks Builder Elements
	 */
	public function get_bricksbuilder_elements_array() {
		$elements         = array();
		$elements_exclude = array(
			'container',
			'section',
		...
		$exclude_elements = array_diff( $elements, $elements_exclude );
